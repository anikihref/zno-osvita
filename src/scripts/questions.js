/*
В каждом вопросе обязательно должно быть поле type

Если нужно выбрать ответ то поле type в вопросе 'radio'
Внутри также могут быть поля:
		> text (необязательное) - 	просто текст вопроса в основном используется в одиночных вопросах
		либо для уточнения вопроса (можно использовать html тэги).
		"У Пети было <b>100</b> рублей, у Димы - <b>120</b>. Сколько у них было вместе?"

		> questions (необязательное) - 	массив в котором записаны вопросы если их несколько (можно использовать html тэги).
		Используется если выбрать нужно для нескольких вопросов ответы
		['Сколько будет ...', 'Какой будет <b>x</b> если ...']

		> questionImage (необязательное) -	строка в которой лежит путь к нужной для вопроса картинке картинке.
		"/svg/algebra_rivniannia/Year2021/dodatkova/zavdannia-9.svg"

		> variants (обязательное) -	массив с вариантами ответов (можно использовать html тэги).
		['<b>2</b>', <img src="/svg/algebra_rivniannia/Year2021/dodatkova/zavdannia-9.svg">, '42']

		> expectedAnswer (обязательное) -	массив с правильными ответами либо одним ответом (последовательность важна).
		Например: 
			У нас есть 3 варианта ответа написанные в поле variants:
			> ['<b>2</b>', <img src="/svg/algebra_rivniannia/Year2021/dodatkova/zavdannia-9.svg">, '42']

			И 2 вопроса неважно какие, важно знать ответ на них (припустим для вопроса 1 правильный вариант 2, а для 2 - 1). Тогда:
			expectedAnswer: ["<img src="/svg/algebra_rivniannia/Year2021/dodatkova/zavdannia-9.svg">", "<b>2</b>"]

--------------------------------------------------------------------------------------------------------------------------------

Если нужно вписать ответ то поле type в вопросе 'write'
Внутри также могут быть поля:
		> text (необязательное) - 	просто текст вопроса в основном используется в одиночных вопросах
		либо для уточнения вопроса (можно использовать html тэги).
		"У Пети было <b>100</b> рублей, у Димы - <b>120</b>. Сколько у них было вместе?"
		...
		> expectedAnswer (обязательное) - массив с правильными ответами либо одним ответом (последовательность важна).
		Например: 
			У нас есть 2 вопроса в поле questions:
			> ['Сколько будет 21 + 21', 'И ещё там какой-то вопрос']

			Тогда:
			expectedAnswer: ["42", "Какой-то там ответ"]
*/
